...
export default function tensionCalcScreen() {
  ...
  const { setTensionData } = useAppContext();
  ...
  const [isqWeightMetric, setqWeightMetric] = React.useState(true); //kg
  const [isyMidMetric, setyMidMetric] = React.useState(true); //m
  const [islSpanMetric, setlSpanMetric] = React.useState(true); //m
  const [ispLoadMetric, setpLoadMetric] = React.useState(true); //kg
    ...
    <ParallaxScrollView
      ...
      <ThemedView style={styles.stepContainer}>
        ...
        <ThemedText type="default" style={styles.formula}>
          % Tension = ( (lSpan * pLoad) / (4 * yMid) ) + ( (qWeight * lSpan^2) / (8 * yMid) )
        </ThemedText>
